window.onload = function() {
    if (window.navigator.platform.indexOf("Win") != -1) { //windows浏览器测试.
        document.documentElement.style.fontSize = document.documentElement.clientWidth / 7.5 + "px"; //设置字体为设计稿
        imready();
    } else { //手机测试
    }
}
apiready = function() {
    ...
    //TODO 此处用于埋点.所有需要埋点的界面都需要引入common.js并实现imready方法.
    //如果api.frameName不为空,则为对应的frame页面,
    //如果为空,则为window页面,统计所有window页面即可.
    if (!api.frameName) { //win
        //	 	var baiduMTJ = api.require('baiduMTJ');
        //		baiduMTJ.onPageStart({
        //		    pagename: api.winName,
        //		}, function(ret, err) {
        //		    console.log(api.winName+"页面打开"+JSON.stringify(ret))
        //		});
    }
    imready();
}
...
function closeWin() {
    if (!api.frameName) { //win
        ...
        //		    pagename: api.winName,
        //		}, function(ret, err) {
        //		    console.log(api.winName+"页面关闭"+JSON.stringify(ret))
        //		});
    }
    api.closeWin({
        name: api.winName
...
function handelVersionBelow4(style) {
    ...
    if (api.systemType == "android" && splits[0] <= 4 && splits[1] < 4) {
        style.top = 0;
    } //如果版本是andrid就设置顶部的距离为0设置沉浸式低栏
}

function openWin(name, dir, pageParam) {
    //封装了打开win的方法url用三木运算判断
    api.openWin({
        name: name,
 ...
 */
function openWinWithTitle(name, title, frameInfo, dir) {
    api.openWin({
        name: name,
        url: dir + 'title.html',
        pageParam: {
...
function checkRequestIp(callBack) {
    ...
    if (!requestIp || requestIp == '0.0.0.0') {
        api.ajax({
            url: 'http://ip.taobao.com/service/getIpInfo.php?ip=myip' //获取IP地址
        }, function(ret, err) {
            if (ret) {
                if (ret.code == 0) {
...
function enHanceBody(bodyParam) {
    ...
    var tradeId = hex_md5(api.deviceId); //md5加密的设备唯一标示
    var version = "1.0" //软件版本
    ...
    userInfo.refer = "";
    userInfo.appToken = api.deviceId; //设备唯一标识
    ...
    var s = "appkey=" + appkey + "requestIp=" + requestIp + "timestamp=" + timestamp + "tradeId=" + tradeId +
...
function ajaxRequest(bodyParam, callBack) {
    //雷红本测试地址
//  	var LHBPath = 'http://192.168.1.111:8082/gateway';
    //张国华测试接口
    // var Zghpath = "http://192.168.1.107:8181/gateway";
    // var path ="http://192.168.1.78:8086/gateway"
    enHanceBody(bodyParam)
    api.getPrefs({
        sync: false,
        key: 'serviceUrl'
    }, function(ret, err) {
//      var common_url=LHBPath;
       var common_url = ret.value;
        api.ajax({
            url: common_url,
            method: "POST",
            cache: false,
...
function ajaxRequestWithUrl(common_url, bodyParam, callBack) {
    //总服务器地址
    enHanceBody(bodyParam)
...
Refresh = function(callback) {
    ...
    //打印数据
c = function(tag,value) {
    console.log(tag+JSON.stringify(value));

};
goRequest=function(methodName,requestMsg,callback){
  var data = {}
  data.method = methodName;
  data.request =requestMsg;
  console.log(methodName+"请求数据" + JSON.stringify(data));
  api.showProgress();
  ajaxRequest(data, function(ret, err) {
      console.log(methodName+"成功返回数据" + JSON.stringify(ret));
      api.hideProgress();
      if (ret) {
